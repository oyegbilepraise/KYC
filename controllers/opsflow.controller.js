...
const create_observer = async (req, res) => {
  ...
  try {
    const response = await axios.post(`${BASE_URL}observer`, {
      fname,
      lname,
      email,
    });
    res
      ...
      .json({
        ...
        error: false,
        data: response.data.data,
        message: "created observer successfully",
      });
  } catch (error) {
    res
      .status(500)
...
const register_observer_to_track = async (req, res) => {
  ...
  try {
    const response = await axios.post(`${BASE_URL}observer/${track_id}/track`, {
      track_id,
    });
    res
      .status(200)
      .json({
        ...
        error: false,
        data: response.data.data,
        message: "obect - track successful",
      });
  } catch (error) {
    res
      .status(500)
...
const push_object_to_track = async (req, res) => {
  ...
  try {
    const response = await axios.post(`${BASE_URL}rail`, {
      track_id,
      o_identifier,
    });
    res
      ...
      .json({
        ...
        error: false,
        data: response.data.data,
        message: "Object pushed to track successfully",
      });
  } catch (error) {
    res
      .status(500)
...
module.exports = {
  ...
  create_observer,
  create_bucket,
  register_observer_to_track,
  push_object_to_track,
};
