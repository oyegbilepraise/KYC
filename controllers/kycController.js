...
const kyc = async (req, res) => {
  ...
  let list = [
    ...
    { id: "c-bvn", title: "Credit Bureau/Bvn" },
  ];
  ...
  try {
    if (step == 0 && stage == 0 && trimmed_res == "field") {
      ...
      if (user) {
        ...
        await KYC.update(
          { step: 1, stage: 0, location: user?.data?.id },
      ...
      if (response === "lead") {
        ...
        message = `Inflow Amount: ${data.inflows || 0}, \nOutflow Amount: ${
          ...
          data.onboarded_merchants_count
        }, \nTeam Members: ${
          data.team_members.length || 0
        } \n\n Click on the link below for details \n\n ${body?.data?.url}`;
        await KYC.update({ step: 0, stage: 0 }, { where: { id: starting.id } });
      } else if (response === "claim") {
        const claim = await callClaimMerchant(
          "claim",
          phone,
          provider,
          channelId
        ...
        let messages = `Please choose an option`;
        ...
        const claim = await callBookCredit(
          "statement",
          phone,
          provider,
          channelId
        ...
        const claim = await callOnboardMerchant(
          "merchant",
          phone,
          provider,
          channelId
      ...
      if (data.status) {
        if (phone === data?.data?.mobile) {
          let messages =
            "You can not add yourself as team lead. Please enter your team lead's number";
          message = await interactive.List(messages, list);
        } else {
          let messages = `The Team lead name is ${data?.data?.full_name}`;
          message = await interactive.productsButtons(
            messages,
            [
          ...
          step++;
          await KYC.update(
            { step, other_name: data?.data?.mobile },
            { where: { id: starting.id } }
          );
        }
      } else {
        let messages =
          "This number does not belong to any team lead, please try again";
        message = await interactive.List(messages, list);
      }
...
const hygeia_schedule = async (req, res) => {
  ...
  try {
    const response = await axios.post(
      "https://mobile.creditclan.com/api/v3/loan/recovery",
